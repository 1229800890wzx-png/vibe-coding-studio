...
export default ({command, mode}: ConfigEnv): UserConfig => {
    ...
    return {
        ...
        plugins: [
            ...
            {
                name: 'vibe-active-domain-boundary',
                apply: 'build',
                generateBundle() {
                    const inactive = /\/src\/views\/(ai|bpm|erp|fms|hrm|im|iot|mes|mp|oa|pms|report|wms)\//
                    for (const id of this.getModuleIds()) {
                        const modulePath = normalizePath(id)
                        if (modulePath.includes('dhtmlx-gantt') || inactive.test(modulePath)) {
                            this.error(`Inactive upstream module entered this release: ${modulePath}`)
                        }
                    }
                }
            }

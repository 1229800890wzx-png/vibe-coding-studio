<template>
  <ContentWrap>
    <el-alert
      v-if="formData.order.deliveryType === 3"
      :title="
        formData.entitlementAction === 'KEEP'
          ? '课程售后：退款成功后保留该学员报名与学习权益，不释放班级名额。'
          : formData.entitlementAction === 'CANCEL'
            ? '课程售后：退款成功后取消该学员报名与学习权益，按当前班级释放名额。'
            : '课程售后：权益处理方式尚未返回，请刷新确认。'
      "
      :type="formData.entitlementAction === 'KEEP' ? 'info' : 'warning'"
      :closable="false"
      show-icon
      class="mb-20px"
    />
    ...
    <el-descriptions title="订单信息">
      ...
      <el-descriptions-item v-if="formData.order.deliveryType !== 3" label="收货人: ">
      ...
      <el-descriptions-item v-if="formData.order.deliveryType !== 3" label="联系电话: ">
    ...
    <el-descriptions title="售后信息">
      <el-descriptions-item v-if="formData.studentName" label="报名学员: ">{{
        formData.studentName
      }}</el-descriptions-item>
      <el-descriptions-item v-if="formData.currentCohortName" label="当前班级: ">{{
        formData.currentCohortName
      }}</el-descriptions-item>
      <el-descriptions-item v-if="formData.entitlementAction" label="学习权益处理: ">{{
        formData.entitlementAction === 'KEEP' ? '保留报名与学习权益' : '取消报名与学习权益'
      }}</el-descriptions-item>
    ...
    <el-descriptions :column="1" title="退款状态">
      ...
      <el-descriptions-item>
        ...
        <template v-if="formData.order.deliveryType === 3"
          >请核对学员、当前班级、退款金额及权益处理方式。教学服务无需退货物流，权益变化在退款成功后生效。</template
        >
        <template v-else
          >如果未发货，请点击同意退款给买家。<br />
          如果实际已发货，请主动与买家联系。<br />
          如果订单整体退款后，优惠券和余额会退还给买家。</template
        >
      </el-descriptions-item>
    </el-descriptions>

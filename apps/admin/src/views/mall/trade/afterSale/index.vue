<template>
  ...
  <ContentWrap>
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="学员 / 当前班级" min-width="190">
        <template #default="{ row }"
          ><div>{{ row.studentName || '—' }}</div
          ><small>{{ row.currentCohortName || '' }}</small></template
        >
      </el-table-column>
      <el-table-column label="退款后的学习权益" min-width="180">
        <template #default="{ row }"
          ><el-tag
            v-if="row.entitlementAction"
            :type="row.entitlementAction === 'CANCEL' ? 'danger' : 'success'"
            >{{
              row.entitlementAction === 'CANCEL' ? '取消报名与学习权益' : '保留报名与学习权益'
            }}</el-tag
          ><span v-else>—</span></template
        >
      </el-table-column>
      <el-table-column align="right" label="退款金额" min-width="120" prop="refundPrice">

...
public class CrmPermissionServiceImpl implements CrmPermissionService {
    ...
    public void updatePermission(CrmPermissionUpdateReqVO updateReqVO) {
        ...
        // The controller authorizes this business record. Supplied permission IDs must belong to it.
        List<CrmPermissionDO> existing = permissionMapper.selectByIds(updateReqVO.getIds());
        if (isOwner(updateReqVO.getLevel()) || existing.stream().anyMatch(permission ->
                !Objects.equals(permission.getBizType(), updateReqVO.getBizType())
                || !Objects.equals(permission.getBizId(), updateReqVO.getBizId()) || isOwner(permission.getLevel()))) {
            throw exception(CRM_PERMISSION_DENIED, CrmBizTypeEnum.getNameByType(updateReqVO.getBizType()));
        }
        // 2. 更新
    ...
    public void deletePermissionBatch(Collection<Long> ids, Long userId) {
        ...
        if (convertSet(permissions, CrmPermissionDO::getBizId).size() > 1
                || convertSet(permissions, CrmPermissionDO::getBizType).size() > 1) {

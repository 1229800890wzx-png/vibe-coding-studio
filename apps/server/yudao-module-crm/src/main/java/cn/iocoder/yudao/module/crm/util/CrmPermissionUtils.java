...
public class CrmPermissionUtils {
    ...
    public static <T extends MPJLambdaWrapper<?>, S> void appendPermissionCondition(T query, Integer bizType, SFunction<S, ?> bizId,
        ...
        // “全部” is all readable records, not a bypass of the original CRM owner/team scope.
        if (sceneType == null) {
            if (isCrmAdmin()) return;
            AdminUserApi adminUserApi = SpringUtil.getBean(AdminUserApi.class);
            var readableUsers = new HashSet<>(convertSet(adminUserApi.getUserListBySubordinate(userId), AdminUserRespDTO::getId));
            readableUsers.add(userId);
            query.leftJoin(CrmPermissionDO.class, on -> on.eq(CrmPermissionDO::getBizType, bizType)
                    .eq(CrmPermissionDO::getBizId, bizId).in(CrmPermissionDO::getUserId, readableUsers)
            ...
            query.and(scope -> scope.isNull(ownerUserIdField).or().in(ownerUserIdField, readableUsers)
                    .or().isNotNull(CrmPermissionDO::getId));
            query.distinct(); // Several readable team members must not duplicate one business record.
            return;
        }
        // 场景一：我负责的数据

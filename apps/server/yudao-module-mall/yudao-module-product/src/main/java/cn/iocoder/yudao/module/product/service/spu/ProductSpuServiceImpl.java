...
public class ProductSpuServiceImpl implements ProductSpuService {
    ...
    private org.springframework.beans.factory.ObjectProvider<ProductBusinessGuard> businessGuards;
    ...
    public void updateSpu(ProductSpuSaveReqVO updateReqVO) {
        businessGuards.orderedStream().forEach(guard -> guard.beforeCatalogMutation(updateReqVO.getId()));
        // 校验 SPU 是否存在
    ...
    private void initSpuPrices(ProductSpuDO spu, List<ProductSkuSaveReqVO> skus) {
        // sku 单价最低的商品的价格
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void refreshSpuPrice(Long spuId) {
        // Use a current read even under MySQL REPEATABLE READ. The caller's course lock
        // serializes education catalog mutations; stock remains owned by updateSpuStock.
        var skus = productSkuMapper.selectList(new LambdaQueryWrapper<ProductSkuDO>()
                .eq(ProductSkuDO::getSpuId, spuId).orderByAsc(ProductSkuDO::getId).last("FOR UPDATE"));
        if (skus.isEmpty()) return;
        ProductSpuDO update = new ProductSpuDO().setId(spuId);
        initSpuPrices(update, BeanUtils.toBean(skus, ProductSkuSaveReqVO.class));
    ...
    public void deleteSpu(Long id) {
        businessGuards.orderedStream().forEach(guard -> guard.beforeCatalogMutation(id));
        // 校验存在

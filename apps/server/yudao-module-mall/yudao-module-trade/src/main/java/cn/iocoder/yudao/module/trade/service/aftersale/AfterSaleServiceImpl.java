...
public class AfterSaleServiceImpl implements AfterSaleService {
    ...
    @Resource private TradeOrderMapper educationOrderMapper;
    @Resource private TradeOrderItemMapper educationItemMapper;
    ...
    public Long createAfterSale(Long userId, AppAfterSaleCreateReqVO createReqVO) {
        ...
        educationPolicies.orderedStream().forEach(p -> p.beforeAfterSaleCreate(userId, tradeOrderItem, createReqVO));

        // 第二步，存储售后订单
        AfterSaleDO afterSale = createAfterSale(createReqVO, tradeOrderItem);
        educationPolicies.orderedStream().forEach(p -> p.afterAfterSaleCreate(afterSale, createReqVO.getEntitlementAction()));
    ...
    private TradeOrderItemDO validateOrderItemApplicable(Long userId, AppAfterSaleCreateReqVO createReqVO) {
        ...
        educationOrderMapper.selectOneForUpdate(TradeOrderDO::getId, orderItem.getOrderId());
        orderItem = educationItemMapper.selectOneForUpdate(TradeOrderItemDO::getId, orderItem.getId());
        final Long lockedItemId = orderItem.getId();
        boolean education = educationPolicies.orderedStream().anyMatch(p -> p.isEducationItem(lockedItemId));
        // 已申请售后，不允许再发起售后申请
        if (Objects.equals(orderItem.getAfterSaleStatus(), TradeOrderItemAfterSaleStatusEnum.APPLY.getStatus()) || (!education && !TradeOrderItemAfterSaleStatusEnum.isNone(orderItem.getAfterSaleStatus()))) {
            throw exception(AFTER_SALE_CREATE_FAIL_ORDER_ITEM_APPLIED);
        }
        long reserved = 0;
        // A current locking read is required: the ownership read above may have established an older REPEATABLE READ snapshot.
        for (AfterSaleDO prior : tradeAfterSaleMapper.selectList(new com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<AfterSaleDO>().eq(AfterSaleDO::getOrderItemId, orderItem.getId()).last("FOR UPDATE"))) {
            if (AfterSaleStatusEnum.APPLYING_STATUSES.contains(prior.getStatus())) throw exception(AFTER_SALE_CREATE_FAIL_ORDER_ITEM_APPLIED);
            if (Objects.equals(prior.getStatus(), AfterSaleStatusEnum.COMPLETE.getStatus())) reserved += prior.getRefundPrice();
        }
        if (createReqVO.getRefundPrice() <= 0 || reserved + createReqVO.getRefundPrice() > orderItem.getPayPrice()) throw exception(AFTER_SALE_CREATE_FAIL_REFUND_PRICE_ERROR);
        // 申请的退款金额，不能超过商品的价格
    ...
    private AfterSaleDO createAfterSale(AppAfterSaleCreateReqVO createReqVO,
        ...
        // Education permits sequential partial refunds; normalize only after the locked balance check.
        if (educationPolicies.orderedStream().anyMatch(p -> p.isEducationItem(orderItem.getId()))) educationItemMapper.updateById(new TradeOrderItemDO().setId(orderItem.getId()).setAfterSaleStatus(TradeOrderItemAfterSaleStatusEnum.NONE.getStatus()));
        // 更新交易订单项的售后状态
    ...
    public void refundAfterSale(Long userId, String userIp, Long id) {
        ...
        educationOrderMapper.selectOneForUpdate(TradeOrderDO::getId, afterSale.getOrderId());
        educationItemMapper.selectOneForUpdate(TradeOrderItemDO::getId, afterSale.getOrderItemId());
        afterSale = tradeAfterSaleMapper.selectOneForUpdate(AfterSaleDO::getId, id);
        if (afterSale.getPayRefundId() != null) return; // original refund already dispatched
        Integer newStatus;
        if (ObjUtil.equals(afterSale.getRefundPrice(), 0)) {
            ...
            final AfterSaleDO completed = afterSale;
            educationPolicies.orderedStream().forEach(p -> p.afterRefundSuccess(completed));
            tradeOrderUpdateService.updateOrderItemWhenAfterSaleSuccess(afterSale.getOrderItemId(), 0);
    ...
    public void updateAfterSaleRefunded(Long id, Long orderId, Long payRefundId) {
        ...
        if (!Objects.equals(afterSale.getOrderId(), orderId)) throw exception(AFTER_SALE_REFUND_FAIL_REFUND_ORDER_ID_ERROR);
        educationOrderMapper.selectOneForUpdate(TradeOrderDO::getId, afterSale.getOrderId());
        educationItemMapper.selectOneForUpdate(TradeOrderItemDO::getId, afterSale.getOrderItemId());
        afterSale = tradeAfterSaleMapper.selectOneForUpdate(AfterSaleDO::getId, id);
        if (Objects.equals(afterSale.getStatus(), AfterSaleStatusEnum.COMPLETE.getStatus()) && Objects.equals(afterSale.getPayRefundId(), payRefundId)) return;
        if (ObjectUtil.notEqual(afterSale.getStatus(), AfterSaleStatusEnum.WAIT_REFUND.getStatus())) {
            throw exception(AFTER_SALE_REFUND_FAIL_STATUS_NOT_WAIT_REFUND);
        }
    ...
    private PayRefundRespDTO validatePayRefund(AfterSaleDO afterSale, Long payRefundId) {
        if (payRefundId == null || !Objects.equals(afterSale.getPayRefundId(), payRefundId)) throw exception(AFTER_SALE_REFUND_FAIL_REFUND_ORDER_ID_ERROR);
        // 1. 校验退款单是否存在
        ...
        // 2.3 校验退款订单匹配（二次）
        if (ObjectUtil.notEqual(payRefund.getMerchantRefundId(), afterSale.getId().toString()) || !Objects.equals(payRefund.getMerchantOrderId(), String.valueOf(afterSale.getOrderId()))) {

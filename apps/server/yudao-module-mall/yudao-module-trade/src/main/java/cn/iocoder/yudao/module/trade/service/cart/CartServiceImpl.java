...
public class CartServiceImpl implements CartService {
    ...
    @Resource
    private TradeEducationValidator educationValidator;

    @Override
    @Transactional(rollbackFor = Exception.class)
    public Long addCart(Long userId, AppCartAddReqVO addReqVO) {
        boolean education = educationValidator.validateItem(userId, addReqVO.getSkuId(),
                addReqVO.getStudentId(), addReqVO.getCount());
        // 查询 TradeCartDO
        CartDO cart = cartMapper.selectByUserIdAndSkuIdAndStudentId(userId, addReqVO.getSkuId(), addReqVO.getStudentId());
        // 校验 SKU
        Integer count = addReqVO.getCount();
        int mergedCount = education ? 1 : count + (cart == null ? 0 : cart.getCount());
        ProductSkuRespDTO sku = checkProductSku(addReqVO.getSkuId(), mergedCount);
        checkCombinedCartStock(userId, sku, cart == null ? null : cart.getId(), mergedCount);
        ...
        // 情况二：不存在，则进行插入
            ...
            cart = new CartDO().setUserId(userId).setSelected(true)
                    .setSpuId(sku.getSpuId()).setSkuId(sku.getId()).setStudentId(addReqVO.getStudentId()).setCount(mergedCount);
    ...
    public void updateCartCount(Long userId, AppCartUpdateCountReqVO updateReqVO) {
        ...
        // 校验商品 SKU
        educationValidator.validateItem(userId, cart.getSkuId(), cart.getStudentId(), updateReqVO.getCount());
        ProductSkuRespDTO sku = checkProductSku(cart.getSkuId(), updateReqVO.getCount());
        checkCombinedCartStock(userId, sku, cart.getId(), updateReqVO.getCount());
    ...
    public void resetCart(Long userId, AppCartResetReqVO resetReqVO) {
        ...
        // 第二步：添加新的购物项
        Long studentId = resetReqVO.getStudentId() != null ? resetReqVO.getStudentId() : oldCart.getStudentId();
        educationValidator.validateItem(userId, resetReqVO.getSkuId(), studentId, resetReqVO.getCount());
        CartDO newCart = cartMapper.selectByUserIdAndSkuIdAndStudentId(userId, resetReqVO.getSkuId(), studentId);
        if (newCart != null) {
            ...
            addCart(userId, new AppCartAddReqVO().setSkuId(resetReqVO.getSkuId())
                    .setStudentId(studentId).setCount(resetReqVO.getCount()));
        }
    }
    ...
    public AppCartListRespVO getCartList(Long userId) {
        ...
        // A closed/already purchased course is one invalid cart row, not a failed entire cart page.
        for (Iterator<AppCartListRespVO.Cart> iterator = result.getValidList().iterator(); iterator.hasNext();) {
            AppCartListRespVO.Cart row = iterator.next();
            CartDO cart = carts.stream().filter(item -> Objects.equals(item.getId(), row.getId())).findFirst().orElseThrow();
            try {
                educationValidator.validateItem(userId, cart.getSkuId(), cart.getStudentId(), cart.getCount());
            } catch (ServiceException businessFailure) {
                row.setInvalidReason(businessFailure.getMessage());
                iterator.remove();
                result.getInvalidList().add(row);
            }
        }
        return result;
    }
     ...
     * @return 商品 SKU
     */
    private void checkCombinedCartStock(Long userId, ProductSkuRespDTO sku, Long replacingId, int count) {
        long total = count + cartMapper.selectListByUserId(userId).stream()
                .filter(item -> Objects.equals(item.getSkuId(), sku.getId()) && !Objects.equals(item.getId(), replacingId))
                .mapToLong(CartDO::getCount).sum();
        if (total > sku.getStock()) throw exception(SKU_STOCK_NOT_ENOUGH);
    }

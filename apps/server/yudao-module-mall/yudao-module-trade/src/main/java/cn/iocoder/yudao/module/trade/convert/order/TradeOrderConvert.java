...
public interface TradeOrderConvert {
    ...
    default ProductSkuUpdateStockReqDTO convertStock(List<TradeOrderItemDO> list, int direction) {
        Map<Long, Integer> quantities = new LinkedHashMap<>();
        list.forEach(item -> quantities.merge(item.getSkuId(), item.getCount(), Math::addExact));
        List<ProductSkuUpdateStockReqDTO.Item> items = new ArrayList<>();
        quantities.forEach((skuId, quantity) -> items.add(new ProductSkuUpdateStockReqDTO.Item()
                .setId(skuId).setIncrCount(direction * quantity)));
        ...
        for (AppTradeOrderSettlementReqVO.Item item : settlementReqVO.getItems()) {
            ...
            if (item.getSkuId() != null) {
                reqBO.getItems().add(new TradePriceCalculateReqBO.Item().setSkuId(item.getSkuId()).setCount(item.getCount())
                        .setStudentId(item.getStudentId()).setSelected(true)); // true 的原因，下单一定选中
                continue;
            }
            // 情况二：cartId
            ...
            reqBO.getItems().add(new TradePriceCalculateReqBO.Item().setSkuId(cart.getSkuId()).setCount(cart.getCount())
                    .setStudentId(cart.getStudentId()).setCartId(item.getCartId()).setSelected(true)); // true 的原因，下单一定选中
        }
        return reqBO;
    }

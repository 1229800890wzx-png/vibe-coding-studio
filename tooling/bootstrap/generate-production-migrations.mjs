...
for(const [file,content]of Object.entries(files)){
  ...
  if(check){assert.equal(fs.readFileSync(p,'utf8'),content,`Production baseline drift: ${file}; append a released migration or explicitly refresh the unreleased baseline`);continue;}
...
console.log(`${check?'Verified':'Generated'} production Liquibase baseline: ${Object.values(manifest.tableCounts).reduce((a,b)=>a+b,0)} original/education tables; no development fixtures.`);

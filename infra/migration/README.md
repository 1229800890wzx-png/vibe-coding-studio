...
[source-manifest.json](source-manifest.json) 记录固定公开源码、结构覆盖、每个 SQL/XML/POM 的 SHA256。当前共 10 个变更集；001–008 为冻结基线，009–010 为前向追加：
...
001–008 已作为不可变基线保留，009–010 以追加 changeset 交付。扩展包存在时生成器会拒绝普通生成和 `--refresh-unreleased`，避免重建基线或覆盖后续迁移。后续结构变化只能新增 changeset；任何真实环境应用后都不能修改历史文件、logicalFilePath 或清除 checksum。
...
- 184 张业务/教育表与两个 Liquibase 元数据表全部创建；10 个变更集有引擎校验和且锁已释放。
